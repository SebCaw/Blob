...
function viewFor(state, viewerId) {
  ...
  const players = state.players.map((p) => {
    ...
    return {
      ...
      // would be a lie, and it is not one the table could check. WHICH of the
      // three ways it plays at that level is private and stays on the server.
  ...
  return {
    // Which game this is. A state saved before there was more than one is Blob.
    game: state.game || 'blob',
    id: state.id,
    code: state.code,
    ...
    round: round
      ? {
          ...
          lastTrick: online && round.tricksPlayed && round.tricksPlayed.length
            ...
            : null,
          // Every card played this hand, in order. Public by the same reasoning
          // as the trick on the table: it was all face up, everybody watched it
          // go down. It is what lets a bot remember the hand without being
          // handed anything a player could not have counted themselves.
          trickLog: online && round.tricksPlayed ? round.tricksPlayed : null,

...
function createGame({ hostName, code }, ctx) {
  ...
  const state = {
     ...
     *
     * On by default and the Master's to turn off before the deal. With it off
     * the screen stops ringing your playable cards and stops lighting up the
     * slots a lifted card will go to - you work it out, the way you would at a
     * table with real cards.
     *
     * It is a convenience rather than a secret, so the legal moves still ride
     * in the view: an unplayable card still says what it is waiting for when you
     * press it. That is deliberate. A control that gives nothing back when you
     * press it reads as broken, which has already cost this project a wrong
     * diagnosis once, and turning the hints off should make the game harder
     * rather than make the screen feel faulty.
     */
    hints: true,
    turnId: null,
     ...
     *
     * Going out does NOT end the game - it ends YOUR game. Everybody else plays
     * on, and the last person still holding cards is the loser. Same shape as
     * Sevens and Silly Head, and for the same reason: by the time somebody has
     * gone out, what is left at stake is not being last.
     */
    finished: [],
    /** Whoever was still holding cards when everybody else had gone. */
    loserId: null,
    /** Who won: the first person out. */
    winnerIds: [],
...
function nextTurnId(state, fromId) {
  ...
  for (let step = 1; step <= order.length; step++) {
    ...
    if (id && !isOut(state, id)) return id;
  }
  return null;
}
...
const HANDLERS = {
  ...
  /**
   * The Master decides whether the app points out your legal moves.
   *
   * Lobby only, like Silly Head's `quick`. Changing it mid-game would move the
   * goalposts on whoever is halfway through a turn.
   */
  'game/setHints'(state, cmd, ctx) {
    if (!isMaster(state, ctx.actorId)) return fail('Only the Master can change that.', 'not-master');
    if (state.phase !== 'lobby') return fail('That can only be changed before the game starts.');
    state.hints = cmd.hints !== false;
  },
  ...
  'game/start'(state, cmd, ctx) {
    ...
    // Who leads is drawn from the same seed as the deal rather than being the
    // host every time.
    //
    // This matters more here than it looks. Measuring the bot ladder turned up
    // that heads-up between two competent players, whoever goes first wins
    // essentially every game - so "the person who made the room always starts"
    // was quietly handing them the game. Seeded rather than random so a deal is
    // still replayable from the seed alone, which is what makes an argument
    // about a hand checkable afterwards.
    const leader = shuffle(ids.slice(), `${seed}:leader`)[0];
  ...
  'play/card'(state, cmd, ctx) {
    ...
    // Out is out, and the game carries on without you.
    if (!state.hands[id].length) {
      state.finished.push(id);
      state.lastEvent.wentOut = true;
      state.lastEvent.place = state.finished.length;
      goOut(state, ctx, id);
    }
  },
  ...
  'play/movePile'(state, cmd, ctx) {
    ...
    if (!canMovePile(from, to, state.board)) return fail(pileRefusal(state, from, to), 'not-playable');
    if (state.turnMoves >= MAX_TURN_MOVES) return fail('That is enough for one turn.', 'turn-too-long');
...
/**
 * Why that pile will not go there, in the words you would use at a table.
 */
function pileRefusal(state, from, to) {
  const target = state.board[to] || [];
  if (!target.length) {
    return CORNERS.includes(to)
      ? 'Only a pile headed by a king can go into a corner.'
      : 'A pile can only go onto another pile.';
  }
  const want = takes(target[target.length - 1]);
  if (!want) return 'That pile has run down to an ace. Nothing goes under it.';
  return `That pile needs to start with a ${want.red ? 'red' : 'black'} ${want.rank}.`;
}
 ...
 */
function goOut(state, ctx, id) {
  state.turnId = nextTurnId(state, id);
  state.turnPlayed = false;
  state.turnMoves = 0;
  // A card went down, so the table is demonstrably not stuck.
  state.idleTurns = 0;
  if (settleGame(state, ctx)) return;
  advanceAutoPlays(state, ctx);
}
 ...
 *
 * **Going out does not end it.** The first player out has won, and everybody
 * else keeps playing for the thing that is actually at stake by then, which is
 * not being last. So this ends on one player LEFT rather than on one player
 * finished, which is Sevens' shape - and like Sevens, the last hand is the
 * whole joke of the end screen.
 *
 * @returns {boolean} true if the game has just ended
 */
function settleGame(state, ctx) {
  ...
  if (left.length <= 1) {
    ...
    state.winnerIds = state.finished.length ? [state.finished[0]] : [];
  ...
  // down to an ace can never be built on again.
  //
  // Everybody still in is placed by how few cards they hold, which is the
  // nearest honest thing to the order they would have gone out in, and whoever
  // holds the most is the loser. Anybody already out keeps the place they
  // earned.
  if (state.idleTurns >= left.length * IDLE_ROUNDS) {
    const wentOut = state.finished.length;
    const ranked = left
      .map((p) => ({ id: p.id, held: state.hands[p.id].length }))
      .sort((a, b) => a.held - b.held || a.id.localeCompare(b.id));
    for (const entry of ranked) state.finished.push(entry.id);

    const fewest = ranked[0].held;
    const most = ranked[ranked.length - 1].held;
    const holdingMost = ranked.filter((r) => r.held === most);

    state.phase = 'complete';
    state.completedAt = ctx.now;
    // A tie stays a tie. The order above needs one, so it takes an arbitrary
    // tiebreak - but the RESULT does not have to pretend, and a coin toss for a
    // title nobody agreed to play for is worse than an honest draw. Somebody who
    // actually went out still wins outright; only a board that died with nobody
    // out can be shared.
    state.winnerIds = wentOut
      ? [state.finished[0]]
      : ranked.filter((r) => r.held === fewest).map((r) => r.id);
    // And nobody is named the loser when two people are level on the most.
    state.loserId = holdingMost.length === 1 ? holdingMost[0].id : null;

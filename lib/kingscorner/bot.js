...
function thinkMs(view, secret, kind) {
  ...
  // Nudged up on Seb's report that the table moved faster than he could read
  // it. Deliberately a nudge and not a change of pace: a bot that takes long
  // enough to look like it is thinking is good, and one you are waiting on is
  // not. The continuing figure stays much lower because a chain of moves is ONE
  // person's go, and two seconds between each of them reads as four players.
  const base = continuing ? 440 : 950;
  const spread = continuing ? 480 : 1000;
  return Math.round(clamp(base + rng() * spread, continuing ? 380 : MIN_THINK_MS, MAX_THINK_MS));
}

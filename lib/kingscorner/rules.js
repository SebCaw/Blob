 ...
 * back off one.
 *
 * A pile may only land on another pile, never in an empty slot. Relocating a
 * pile to a bare slot would achieve nothing except moving the hole, and it is
 * the one move a bot could make back and forth for ever.
 *
 * @param {string} from
 * @param {string} to
 * @param {Record<string,string[]>} board
...
/**
 * A fingerprint of the board, for a bot's `at` key and for the livelock guard.
 *
 * It is every card in every slot in order, which is exactly what a person
 * looking at the table can see. Two boards a player could not tell apart give
 * the same string.
 *
 * @param {Record<string,string[]>} board
 * @returns {string}
...
function boardKey(board) {
  return SLOTS.map((slot) => `${slot}:${(board[slot] || []).join(',')}`).join('|');
}

...
const { MIN_PLAYERS, MAX_PLAYERS, findPlayer, playableMoves, isOut } = require('./game');
...
function viewFor(state, viewerId) {
  ...
  const players = state.players.map((p) => {
    ...
    return {
      ...
      // Out, and where they finished. 1 is the winner; nobody gets a place for
      // being last, which is what `isLoser` is for.
  ...
  return {
    ...
    stuck,
    // Whether the screen may point out what you can play. The Master's call,
    // and everybody plays under the same one.
    hints: state.hints !== false,
    winnerIds: winners.slice(),
    winnerNames: winners.map((id) => (findPlayer(state, id) || {}).name || 'Someone'),
    // The order people went out in, which is the whole result of this game.
...
function historyRecord(state) {
  ...
  return {
    ...
    players: state.players.map((p) => ({
      id: p.id,
      name: p.name,
      // What they were left holding. Zero for everybody who went out.
...
function historySummary(record) {
  ...
  return {
    ...
    // No rounds and no score. The shape of this game is the order people went
    // out in, so that is what the line says - the same as Sevens'.

...
const CHEAT = {
  ...
  stallWatch(state) {
    if (state.phase !== 'playing' || !state.turnId) return null;
    // An OPEN claim is not a stall: the clock is already running on it, and the
    // player whose turn is next has not been asked for anything yet. A CLOSED
    // one is just the last thing said, so the turn holder is genuinely holding
    // everybody up and the stall watch should say so.
    if (state.claim && !state.claim.closed) return null;
   ...
   */
  deadline(state, now) {
    if (state.phase !== 'playing' || !state.claim || state.claim.closed) return null;
    return {
  ...
  bots: {
    ...
    owing(state) {
      ...
      // Only while the window is OPEN. A claim that has stood is still callable
      // by a person who has worked it out late, and deliberately not by a bot:
      // a bot has already had its say, and one that kept re-reading a settled
      // claim would either never let the game move on or would take a shot
      // nobody at the table could see coming.
      if (state.claim && !state.claim.closed) {
...
const ENGINES = { blob: BLOB, sillyhead: SILLYHEAD, sevens: SEVENS, chase: CHASE, cheat: CHEAT, gofish: GOFISH };

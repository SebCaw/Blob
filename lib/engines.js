...
const gofishGame = require('./gofish/game');
const gofishView = require('./gofish/view');
const gofishBot = require('./gofish/bot');
const { completeBooks: gofishCompleteBooks } = require('./gofish/rules');
const kingscornerGame = require('./kingscorner/game');
const kingscornerView = require('./kingscorner/view');
const kingscornerBot = require('./kingscorner/bot');
...
const GOFISH = {
  id: 'gofish',
  name: 'Go Fish',
  createGame: gofishGame.createGame,
  applyCommand: gofishGame.applyCommand,
  findPlayer: gofishGame.findPlayer,
  viewFor: gofishView.viewFor,
  historyRecord: gofishView.historyRecord,
  historySummary: gofishView.historySummary,

  /**
   * Who the table is stuck behind.
   *
   * Two candidates, like Chase the Ace and for a sharper reason. A turn in this
   * game is taken by TWO people: somebody asks, and somebody else answers. While
   * a question is on the table the person holding everybody up is the target,
   * not the player whose turn it is — and the player whose turn it is cannot do
   * a thing about it. Watching only `turnId` would leave a table frozen behind a
   * phone in a pocket with nothing on any screen to say whose.
   */
  stallWatch(state) {
    if (state.phase !== 'playing') return null;
    const waitingOn = state.ask ? state.ask.targetId : state.turnId;
    if (!waitingOn) return null;
    const player = gofishGame.findPlayer(state, waitingOn);
    const skipping = Boolean(state.autoPlay && state.autoPlay[waitingOn]);
    if (!player || player.left || player.connected || skipping || player.awaitingTakeover) return null;
    return { playerId: waitingOn, command: { type: 'play/stalled', playerId: waitingOn } };
  },

  // No `deadline`. Nothing in Go Fish is on a clock: there is no window to shut
  // and no pause to end, only people taking their turn. The room asks before it
  // calls, so leaving it off is the whole of saying so.

  bots: {
    /**
     * The one thing a bot owes the table right now.
     *
     * Three kinds, and the order between them is the point.
     *
     * ANSWERING first, because an unanswered question holds up everybody —
     * including the player whose turn it is, who has already acted and can do
     * nothing until somebody replies.
     *
     * BOOKING second, and deliberately for ANY bot rather than only the one on
     * turn: a deal can hand somebody four of a rank before a card has moved, and
     * three bots filing their books at the start is the right way for a game to
     * open. Skipped while a question is on the table, because the reducer
     * refuses a book from either side of an open ask.
     *
     * ASKING last, and only for whoever's turn it is.
     *
     * The three `at` keys are built on different things. Answering keys on the
     * moment the question was asked, which is unique to it. Booking keys on the
     * bot's own cards, which is what changes when a book goes down. Asking keys
     * on `state.asks`, the running count of questions — deliberately NOT on
     * anything derived from the position, because with an empty pool a failed
     * ask changes NOTHING about the position at all, and a key that did not move
     * would leave the table sat there for ever. Deliberately not `log.length`
     * either: the log is capped, so it stops changing and takes the bots with
     * it.
     */
    owing(state) {
      if (state.phase !== 'playing') return null;
      ...
      if (state.ask) {
        const target = gofishGame.findPlayer(state, state.ask.targetId);
        if (target && target.isBot) {
          return { playerId: target.id, kind: 'answer', at: `${state.ask.askedAt}` };
        }
      }

      if (!state.ask) {
        const filing = gofishGame
          .activePlayers(state)
          .find((p) => p.isBot && !gofishGame.isOut(state, p.id) && gofishCompleteBooks(state.hands[p.id] || []).length);
        if (filing) {
          return {
            playerId: filing.id,
            kind: 'book',
            rank: gofishCompleteBooks(state.hands[filing.id] || [])[0],
            at: `${filing.id}:${(state.books[filing.id] || []).length}:${(state.hands[filing.id] || []).length}`,
          };
        }
      }

      if (state.ask || !state.turnId) return null;
      const turn = gofishGame.findPlayer(state, state.turnId);
      if (!turn || !turn.isBot) return null;
      return { playerId: turn.id, kind: 'ask', at: `${state.turnId}:${state.asks}` };
    },

    thinkMs(view, secret, kind) {
      return gofishBot.thinkMs(view, secret, kind);
    },
     ...
     *
     * A brain that throws falls back to a legal move read straight off
     * `view.you` — the answer, which is forced anyway; the first book it is
     * holding; or any rank it holds asked of anybody still in. Never null, and
     * never a command the reducer will refuse: a refusal strands the table
     * exactly as hard as a throw would.
     */
    move(view, secret, owed) {
      try {
        const chosen = gofishBot.chooseMove(view, secret, owed);
        if (chosen) return chosen;
      } catch (err) {
        console.error('[blob] a bot could not decide what to ask', err.message);
      }
      if (owed && owed.kind === 'answer') return { type: 'play/answer' };
      if (owed && owed.kind === 'book') {
        const rank = owed.rank || gofishCompleteBooks((view.you && view.you.hand) || [])[0];
        if (rank) return { type: 'play/book', rank };
      }
      return gofishBot.anyAsk(view) || { type: 'play/answer' };
    },
  },
};
...
const KINGSCORNER = {
  id: 'kingscorner',
  name: 'Kings Corner',
  createGame: kingscornerGame.createGame,
  applyCommand: kingscornerGame.applyCommand,
  findPlayer: kingscornerGame.findPlayer,
  viewFor: kingscornerView.viewFor,
  historyRecord: kingscornerView.historyRecord,
  historySummary: kingscornerView.historySummary,

  /**
   * Who the table is stuck behind.
   *
   * One candidate, unlike Go Fish's two: a turn here is taken by exactly one
   * person, however many moves they make inside it. Nobody else is ever holding
   * the game up.
   */
  stallWatch(state) {
    if (state.phase !== 'playing' || !state.turnId) return null;
    const player = kingscornerGame.findPlayer(state, state.turnId);
    const skipping = Boolean(state.autoPlay && state.autoPlay[state.turnId]);
    if (!player || player.left || player.connected || skipping || player.awaitingTakeover) return null;
    return { playerId: state.turnId, command: { type: 'play/stalled', playerId: state.turnId } };
  },
  ...
  bots: {
     ...
     * The one thing a bot owes the table right now.
     *
     * The subtlety here is that a bot may owe SEVERAL moves in a row: a turn is
     * a chain, and the room asks again after each one. So `at` has to change
     * every time a move lands, or the room's early return leaves the bot sat
     * after its first card with the table waiting on it — the silent failure
     * described in ADDING-A-GAME.md, spelled out here because this is the first
     * engine where one move is not one turn.
     *
     * `turnMoves` is exactly that counter and it resets when the seat moves on,
     * so the key is whose turn it is, how far into it they are, and how much is
     * on the board. All three are things a person watching could tell apart, and
     * none of them moves on a reconnect or a name edit.
     */
    owing(state) {
      if (state.phase !== 'playing' || !state.turnId) return null;
      const turn = kingscornerGame.findPlayer(state, state.turnId);
      if (!turn || !turn.isBot) return null;
      const held = (state.hands && state.hands[turn.id] ? state.hands[turn.id].length : 0);
      return {
        playerId: turn.id,
        kind: 'move',
        at: `${state.turnId}:${state.turnMoves || 0}:${held}`,
      };
    },

    thinkMs(view, secret, kind) {
      return kingscornerBot.thinkMs(view, secret, kind);
    },
     ...
     *
     * A brain that throws falls back to a legal move read straight off
     * `view.you.moves` — the first card into the first slot that takes it, or
     * simply ending the turn. Ending a turn is accepted by the reducer in every
     * position, which makes it the one move that can always be the last resort:
     * a refusal would strand the table exactly as hard as the throw did.
     */
    move(view, secret) {
      try {
        const chosen = kingscornerBot.chooseMove(view, secret);
        if (chosen) return chosen;
      } catch (err) {
        console.error('[blob] a bot could not choose a move', err.message);
      }
      const cards = Object.entries(((view.you || {}).moves || {}).cards || {});
      if (cards.length) return { type: 'play/card', cardId: cards[0][0], slot: cards[0][1][0] };
      return { type: 'play/endTurn' };
    },
  },
};

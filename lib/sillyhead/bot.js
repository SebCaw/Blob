...
function scoreGroup(view, group, level, rng, count) {
  ...
  // With no deck left, put down ONE unless it sacks.
  //
  // This is the rule that stops the game going round for ever. Dumping your
  // whole set of a number that nobody can beat does not shed anything: the next
  // player is forced to take the pile, and now THEY have all of them, and they
  // hand the lot straight back. Two bots traded the same three aces for
  // thousands of turns. One at a time, the pile comes back one card lighter
  // every time, and somebody eventually runs out — which is the game.
  //
  // While the deck lasts it is the other way round: everything you put down is
  // replaced, so a fistful of low cards is free to throw.
  //
  // The exception is a rank that MATCHES the top: putting more of those down is
  // not handing anything over, it is building the run towards the four that
  // sacks the pile. Without the exception two bots sat trading 9s for ever,
  // because a 9 on the pile blocks everything above it and only the low cards
  // could move.
  const matchesTop = pile.top && rankOf(pile.top) === rank;
  if (!canSack && !deckAlive && !matchesTop) group.play = 1;
  ...
  if (canSack) {
    score += 30;
  } else if (level !== 'medium' && group.play > 1 && deckAlive) {
    // Shedding duplicates while the deck still replaces them is how you avoid
    // being left holding a pair of kings with an ace showing. Medium never
    // thinks of it, and Impossible does it hardest — every card you get rid of
    // for free now is one you cannot be stranded with later.

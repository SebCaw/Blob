...
/**
 * Cards for Silly Head, which needs more than one deck.
 *
 ...
 * Head is normally played with two decks and up to four at a big table, so the
 * same face turns up more than once and the ids have to say WHICH one.
 *
...
/** The standard game is two decks even at a small table — that is how it is played. */
const MIN_DECKS_STANDARD = 2;
...
function decksFor(playerCount, quick = false) {
  if (quick) return 1;
  return Math.max(MIN_DECKS_STANDARD, Math.ceil(playerCount / PLAYERS_PER_DECK));
}

...
const HANDLERS = {
  ...
  'player/remove'(state, cmd, ctx) {
    ...
    // A seat mid-sort has to be counted as done, or the deal never starts: the
    // sort waits for everybody and a bot inheriting a half-sorted table has no
    // command owed that would finish it.
    if (state.phase === 'sort' && state.sortDone) state.sortDone[target.id] = true;

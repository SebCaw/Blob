...
const HANDLERS = {
   ...
   * Finished sorting.
   *
   * Every pair comes off here — the stacking was only ever a way to hold cards
   * on the table while you drew more — leaving exactly three face-up cards. They
   * do not have to be different numbers; two 10s and a king is a fine finish.
   */
  'sort/done'(state, cmd, ctx) {
    ...
    state.up[id] = state.up[id].map((stack) => {
      if (stack.length > 1) state.hands[id].push(...stack.slice(1));

...
const { deal, suitOf, legalPlays, trickWinner, maxOnlineHandSize } = require('./deck');
...
const HANDLERS = {
  ...
  'player/join'(state, cmd, ctx) {
    ...
    // One more player means fewer cards to go round. Trimming the hand size is
    // friendlier than turning someone away at the door, and the lobby shows the
    // new number straight away.
    if (isOnline(state)) state.startHandSize = Math.min(state.startHandSize, handSizeCeiling(state));
  ...
  'trick/play'(state, cmd, ctx) {
    ...
    const cardId = String(cmd.cardId || '');
    ...
    round.hands[trick.turnId] = hand.filter((c) => c !== cardId);
    trick.plays.push({ playerId: trick.turnId, cardId, at: ctx.now });
    if (!trick.ledSuit) trick.ledSuit = suitOf(cardId);

    if (trick.plays.length < state.players.length) {
      trick.turnId = nextToPlay(state, trick.turnId);
      return;
    }
    ...
    round.tricksPlayed.push({
      ...
      plays: trick.plays.map(({ playerId, cardId: id }) => ({ playerId, cardId: id })),
      winnerId,
    });
    ...
    if (handsEmpty) {
      ...
      return { result: { trickWinnerId: winnerId, roundOver: true } };
    }
    openTrick(state, round, winnerId, ctx);
    return { result: { trickWinnerId: winnerId, roundOver: false } };
  },
...
function dealRound(state, round, ctx) {
  ...
  // real cards, so the same person isn't always first to commit.
  round.leadId = playerIds[round.index % playerIds.length];
}
...
/** The next seat round, in join order — which is the order everyone sees. */
function nextToPlay(state, currentId) {

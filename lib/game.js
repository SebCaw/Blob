...
const eligibleForMaster = (state) =>
  state.players.filter((p) => !p.isOffline && !p.isBot && p.connected && p.id !== state.masterId);
...
function leadIdForRound(state, index) {
  ...
  // Where the rotation STARTS is drawn from the game's own id rather than being
  // seat one every time.
  //
  // The rotation itself is unchanged and is the point: the lead moves a seat
  // per round the way the deal does with real cards. What changed is that round
  // one no longer always belongs to whoever made the room. Blob was the last
  // game on the shelf still handing it out that way - Silly Head, Chase the Ace,
  // Cheat and Go Fish all already draw theirs, and Sevens cannot, because there
  // the seven of diamonds leads and that is a rule rather than a convention.
  //
  // Off `state.id`, which is stable for the life of a game and needs no new
  // field, so a game saved before this still reads back and simply starts
  // somewhere else.
  return ids[(leadOffset(state, ids.length) + index) % ids.length];
}

/**
 * A fixed starting seat for one game, from its id.
 *
 * The same FNV hash Cheat and Go Fish already use for the same job. Pure, and
 * stable: asking twice about the same game gives the same answer, which is what
 * lets `leadIdForRound` stay a function of the round number rather than
 * something that has to be stored.
 */
function leadOffset(state, size) {
  const key = String((state && state.id) || '');
  let h = 2166136261;
...
const HANDLERS = {
   ...
   *
   * Online only, and before the off. A bot has to be dealt a hand, so it needs
   * Blob to be dealing; and it takes a seat like anybody else, so the deck has
   * to stretch to it — which is why the starting hand size trims here exactly
   * as it does when a person arrives.
   *
   * Three things are recorded, and only the first two ever leave the server:
   * that it is a bot at all (hiding that would be a lie the table can't check),
   * which level the Master picked, and a private seed. The seed is what decides
   * WHICH of the three ways to play at that level it uses, and it is rechosen
   * every round from that seed — see `lib/bot.js`. It must never appear in a
   * view, or the whole point of having three of them is gone.
   */
  'player/addBot'(state, cmd, ctx) {
    if (!isMaster(state, ctx.actorId)) return fail('Only the Master can add a bot.', 'not-master');
    if (!isOnline(state)) {
      return fail('Bots need Blob to be dealing. Start an online game to play with them.', 'not-online');
    }
    if (state.phase !== 'lobby') return fail('Bots can only be added before the game starts.');
    if (state.players.length >= MAX_ONLINE_PLAYERS) {
      return fail(`Blob can deal to ${MAX_ONLINE_PLAYERS} players at most. This game is full.`, 'game-full');
    }
    ...
    /** Private. How this one plays, and never in a view. */
    ...
    if (state.phase !== 'lobby') {
      ...
      if (state.masterId === player.id) {
        state.masterId = seniority(state).find((id) => {
          ...
          return p && !p.left && !p.isOffline && !p.isBot;
        }) || remaining[0].id;
      }
      return;
    }

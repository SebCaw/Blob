 ...
 *
 * **These are set by how long it takes to READ the table, not by how long a
 * decision should plausibly take.** The first version was paced on the second
 * of those and Seb's verdict after one game was that he did not have time to
 * remember what was going on — which is the right complaint: this is a memory
 * game, everything worth knowing goes past in one line of a transcript, and a
 * bot that answers in 400ms has taken the memory out of it and left the luck.
 *
 * Answering is still the quickest, because there is nothing to decide and the
 * table is watching a person be asked a question. Asking is slower: it is the
 * only real decision in the game and it is also the line everybody has to read.
 * Laying a book is quick, because at the deal three bots may have one and
 * nobody wants to watch them file paperwork.
 *
 * **A beat that told the table something gets longer.** A handover or a book is
 * the moment there is actually something to commit to memory, so the next bot
 * waits noticeably longer before speaking over it. That is the same reasoning
 * as Cheat's pause after a reveal, and it is where most of the readability is.
 */
function thinkMs(view, secret, kind) {
  const rng = rngFrom(`${secret.seed}:pace:${view.version}:${kind}`);
  const fast = view.speed === 2 ? 2 : 1;
  if (kind === 'book') return Math.round(clamp(420 + rng() * 400, 350, 1000) / fast);
  if (kind === 'answer') return Math.round(clamp(900 + rng() * 700, 700, 1900) / fast);

  const last = view.lastEvent && view.lastEvent.kind;
  const toldSomething = last === 'give' || last === 'book';
  const base = toldSomething ? 2100 : 1400;
  const ceiling = toldSomething ? MAX_THINK_MS : 2600;
  return Math.round(clamp(base + rng() * 900, MIN_THINK_MS, ceiling) / fast);
}

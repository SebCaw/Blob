 ...
 *
 * A WINDOW ON A CLOCK, AND A GRACE PERIOD AFTER IT. A play is not finished when
 * it is made — it sits open for a few seconds while anybody may call it. That is
 * the one thing this game cannot copy from a real table, where calling is a
 * shout and the loudest person wins. The window is what makes it fair, and it is
 * why this engine is the first to need `deadline()`: something has to close it
 * when nobody says anything.
 *
 * **But the window closing does not make a claim safe.** It stands, the rank
 * moves on and the turn passes — and it is still callable, right up until the
 * next claim lands on top of it. Seb asked for that after playing: "when people
 * are playing people it takes time to select, so it allows them to click cheat
 * even after the main button has gone". The clock now paces the game rather than
 * policing it, which is the job it is actually good at.
 *
 * The consequence worth understanding is that GOING OUT IS NOT FINAL UNTIL YOU
 * ARE BURIED. Putting your last cards down and surviving the window no longer
 * ends your game: while your claim is still callable it can still be turned
 * over, and a lie called is your whole pile back. You are out once the table has
 * played past you. See `closeWindow` and `buryClaim`.
 *
 ...
 *
 * Four seconds. It was three, and Seb reported the window running out while he
 * was still working out what had been claimed - which is a fair description of
 * one second doing two jobs. Most of that fix is on the glass, where the claim
 * is now the loudest thing on the screen rather than a line of text in the
 * middle of it, but the extra second is cheap and it is the difference between
 * reading and reacting.
 *
 * It is also the beat the animations need: without it a card lands and is gone
 * before anybody registers whose it was.
 */
const CALL_MS = 4_000;
...
/**
 * Whose turn is it after this one? Skips anybody out, gone, or empty-handed.
 *
 * The empty-handed part is not belt and braces, it is load-bearing, and it
 * arrived with the late call. Going out is no longer recorded the moment a
 * window shuts - it waits until the claim is buried, so that the claim can still
 * be turned over - which means there is now a real gap where somebody holds
 * nothing and is not yet marked out. Handing them the turn puts the table on a
 * player who cannot legally play anything, and the game stops. Measured at about
 * one game in forty before this line existed.
 */
function nextTurnId(state, fromId) {
  ...
  for (let step = 1; step <= order.length; step++) {
    ...
    if (!isOut(state, id) && (state.hands[id] || []).length) return id;
  }
  return null;
}
...
function couldCall(state) {
  ...
  // Deliberately does not care whether the window has shut. Who MAY call is a
  // question about the table, not about the clock.
...
const HANDLERS = {
  ...
  'play/claim'(state, cmd, ctx) {
    ...
    // Only an OPEN claim blocks a play. A closed one is waiting to be buried by
    // exactly this command.
    if (state.claim && !state.claim.closed) return fail('There is still a claim on the table.', 'window-open');
    ...
    // Whatever stood last is now out of reach: face down on the pile, and the
    // person who played it is finally safe.
    buryClaim(state, ctx);
    ...
    state.claim = {
      ...
      /**
       * Set when the window shuts. A closed claim has stood — the rank moved on
       * and the turn passed — and is still callable until the next one buries
       * it.
       */
      closed: false,
  ...
  'play/call'(state, cmd, ctx) {
    ...
    // A closed claim is one nobody called in time, and this is the whole of the
    // late call: it is settled exactly the same way, just later.
  ...
  'play/settle'(state, cmd, ctx) {
    ...
    if (!state.claim || state.claim.closed) return;
...
/**
 * The window shuts. The claim stands, the rank moves on, next player.
 *
 * What it deliberately does NOT do is put the cards away. A claim that has stood
 * is still callable until the next one buries it — see the note at the top of
 * this file — so the cards stay where they are and the claimer is not yet out.
 * `buryClaim` does both, later.
 *
 * The distinction is the whole of Seb's late call, and it is worth being exact
 * about: the clock decides when the game MOVES ON, and the next claim decides
 * when the last one is SAFE. They used to be the same moment and they are not
 * the same question.
 */
function closeWindow(state, ctx) {
  const claim = state.claim;
  if (!claim || claim.closed) return;
  claim.closed = true;
  claim.closedAt = ctx.now;
  ...
  // Nobody left who can play. The claim has to go down now: keeping it callable
  // would be keeping a game alive that has no legal move in it, and `settleGame`
  // will not finish while anything is still callable.
  if (!state.turnId) buryClaim(state, ctx);
}

/**
 * Bury the claim that stood: cards face down, and the claimer finally safe.
 *
 * Called from one place — somebody making the next claim — because that is the
 * moment a claim stops being callable. Going out lands here rather than in
 * `closeWindow` for the same reason: while your last cards can still be turned
 * over you are not out of anything.
 */
function buryClaim(state, ctx) {
  const claim = state.claim;
  if (!claim || !claim.closed) return;
  ...
  if ((state.hands[claim.playerId] || []).length === 0 && !isOut(state, claim.playerId)) {
    state.finished.push(claim.playerId);
    note(state, { kind: 'safe', playerId: claim.playerId, at: ctx.now });
  }
}
...
function settleGame(state, ctx) {
  ...
  // Never while anything is still callable. A claim that can still be turned
  // over can still hand its author a pile back, and a game that declared a
  // loser and then had to un-declare one would be worse than a game that waits.
...
function advanceAutoPlays(state, ctx) {
  for (let guard = 0; guard <= state.players.length; guard++) {
    ...
    // An OPEN claim is the table waiting; a closed one is simply the last thing
    // said, and the next player - absent or not - plays straight over it.
    if (state.claim && !state.claim.closed) return;
    ...
    if (state.turnId === before && state.claim && !state.claim.closed) return;
  }
}

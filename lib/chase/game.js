...
const { THE_ACE, DECK_SIZE, deal, minimumDecks, isTheAce } = require('./deck');
const { BOT_LEVELS, BOT_NAMES } = require('./bot');
const { extractPairs, moveCard, insertAt, shuffleHand } = require('./rules');
...
const HANDLERS = {
  ...
  'game/start'(state, cmd, ctx) {
    ...
    // Everybody bins their pairs at once, and the opening discard is public —
    // the room watches it happen at a table, so it is not a secret here either.
    const opened = [];
    for (const id of ids) {
      const { kept, pairs } = extractPairs(state.hands[id]);
      state.hands[id] = kept;
      for (const pair of pairs) {
        state.discarded.push(pair[0], pair[1]);
        opened.push({ playerId: id, pair });
      }
    }
    for (const id of ids) {
      if (!state.hands[id].length) state.finished.push(id);
    }

    state.log = [];
    state.tidied = {};
    note(state, { kind: 'deal', opened, at: ctx.now });
    state.phase = 'playing';
  ...
  'draw/take'(state, cmd, ctx) {
    ...
    const { kept, pairs } = extractPairs(insertAt(state.hands[drawerId], card, roll));
    state.hands[drawerId] = kept;
    ...
    note(state, {
      ...
      fromId,
      index,
      // A pair goes face up in the middle, so both cards are public. A card that
      // did not pair stays where it went, and is named to nobody.
      paired: pairs.length ? pairs[0] : null,
      auto: auto || undefined,
      at: ctx.now,
    });
 ...
 * drawing from it.
 */
function arranger(state, actorId) {
  if (state.phase !== 'playing') return fail('The game is not being played.', 'wrong-phase');
  ...
  if (!(state.hands[actorId] || []).length) return fail('You have no cards.', 'no-cards');
  if (lockedHandId(state) === actorId) {
...
function advanceAutoPlays(state, ctx) {
  for (let guard = 0; guard <= state.players.length; guard++) {
    ...
    const outcome = HANDLERS['draw/take'](state, { index: 0 }, { ...ctx, actorId: null });

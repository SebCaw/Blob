...
/**
 * How long everybody gets to sort their pairs out before anybody may draw.
 *
 * The game used to start the instant it dealt, which meant a bot could reach
 * into your hand about a second after you first laid eyes on thirteen cards you
 * had not read yet. Everybody holds pairs at the deal - that is the shape of the
 * game - so the first thing that happens at a real table is a pause while the
 * room sorts itself out, and there was no equivalent of it here.
 *
 * It ends EARLY the moment nobody still in is holding a pair, so a quick table
 * never waits the full twelve seconds. The cap only exists so one person
 * staring at their cards cannot hold everybody up for ever.
 */
...
function createGame({ hostName, code, decks = 1 }, ctx) {
  ...
  const state = {
    ...
    turnId: null,
    /**
     * While this is set, nobody may draw: everybody is still sorting out the
     * pairs they were dealt. Binning and arranging both work throughout.
     */
    settleFrom: null,
    settleUntil: null,
...
const HANDLERS = {
  ...
  'game/start'(state, cmd, ctx) {
    ...
    // A moment to read your hand before anybody can reach into it.
    state.settleFrom = ctx.now;
    state.settleUntil = ctx.now + SETTLE_MS;
    if (settleGame(state, ctx)) return;
    endSettleIfReady(state);
  ...
  'draw/take'(state, cmd, ctx) {
    ...
    if (state.settleUntil && ctx.now < state.settleUntil) {
      return fail('Everyone is still sorting their pairs out.', 'settling');
    }
    ...
    /**
     * Nor may you take from somebody who is still holding one.
     *
     * The mirror of the rule above, and Seb asked for it after being raided in
     * the gap. Drawing a card that pairs is the good outcome in this game, and
     * without this the reward for it was a moment of being the only player at
     * the table who could not act while everybody else could act on you. There
     * is no cooldown anywhere else in the game to lean on, so the freeze has to
     * be explicit: a hand with an unbinned pair in it is simply closed, in both
     * directions, until the pair has gone.
     *
     * It cannot deadlock a table for long. The holder is blocked from their own
     * turn too, is nudged, and has a button for it; an absent one has it done
     * for them by `advanceAutoPlays`; and `stallWatch` offers the Master the
     * usual way out if they have vanished entirely.
     */
    if (hasPair(state.hands[fromId] || [])) {
      const them = findPlayer(state, fromId);
      return fail(`${them ? them.name : 'They'} still has a pair to throw away.`, 'source-has-pair');
    }
  ...
  /**
   * The settling period is over, so play may start.
   *
   * Dispatched by the room off `deadline()`, with no actor. Carries the moment
   * the period began, so a timer that fires late - after a rematch has dealt a
   * fresh hand, say - is a no-op rather than cutting the new one short.
   */
  'play/begin'(state, cmd, ctx) {
    if (state.phase !== 'playing') return;
    if (!state.settleUntil) return;
    if (cmd.from != null && cmd.from !== state.settleFrom) return;
    state.settleFrom = null;
    state.settleUntil = null;
    advanceAutoPlays(state, ctx);
  },

  /**
   * Somebody the table is waiting on has gone quiet.
   *
   * Honours `cmd.playerId` rather than always assuming the turn-holder, because
   * since a pair closes a hand in both directions the table can now be held up
   * by the player being DRAWN FROM as easily as by the one drawing.
   */
  'play/stalled'(state, cmd, ctx) {
    if (state.phase !== 'playing') return;
    const player = findPlayer(state, cmd.playerId || state.turnId);
...
/**
 * Everybody still in has cleared their pairs, so there is nothing to wait for.
 *
 * Checked rather than timed wherever possible: a table that sorts itself out in
 * four seconds should not sit staring at another eight.
 */
function endSettleIfReady(state) {
  if (!state.settleUntil) return;
  const waiting = stillIn(state).some((p) => hasPair(state.hands[p.id] || []));
  if (waiting) return;
  state.settleFrom = null;
  state.settleUntil = null;
}
...
function advanceAutoPlays(state, ctx) {
  // Bin for every absent player first, not only the one whose turn it is. An
  // absent player holding a pair now BLOCKS the person drawing from them, so
  // clearing only the turn-holder would leave the table stuck behind somebody
  // who is not even here.
  for (const player of activePlayers(state)) {
    if (!state.autoPlay[player.id] || isOut(state, player.id)) continue;
    let safety = 0;
    while (hasPair(state.hands[player.id] || []) && safety++ < 30) {
      const [a, b] = pairIndexes(state.hands[player.id])[0];
      const binned = HANDLERS['hand/bin'](state, { a, b }, { ...ctx, actorId: player.id });
      if (binned && binned.error) break;
      if (state.phase !== 'playing') return;
    }
  }
  ...
  for (let guard = 0; guard <= state.players.length; guard++) {
    ...
    // Nobody draws while the table is still sorting itself out, absent or not.
    if (state.settleUntil) return;

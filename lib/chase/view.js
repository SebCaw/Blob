...
const { pairIndexes, hasPair } = require('./rules');
const {
  MIN_PLAYERS,
  MAX_PLAYERS,
  SETTLE_MS,
...
function viewFor(state, viewerId) {
  ...
  const players = state.players.map((p) => {
    ...
    return {
      ...
      /**
       * They are holding a pair they have not thrown away yet.
       *
       * A boolean, never the cards. Public on purpose: a hand with a pair in it
       * cannot be drawn from, so the table has to be told why it is waiting,
       * and "nobody can take from Fern" is unusable without "because Fern has a
       * pair". It gives away that two of their cards match and nothing about
       * which - and everybody is about to watch those two land in the middle
       * anyway, so it is a second early rather than a secret.
       */
      holdingPair: playing && dealt ? hasPair(hand) : false,
  ...
  const you = viewer
    ? {
        ...
        /** Nobody may draw yet: the table is still sorting its pairs out. */
        settling: playing && Boolean(state.settleUntil),
        canArrange:
          playing &&
  ...
  return {
    ...
    source: drawFromId
      ? {
          ...
          // Closed until they have thrown it away. See `players[].holdingPair`.
          holdingPair: hasPair(state.hands[drawFromId] || []),
        }
      : null,
    lockedId: locked,
    /**
     * The pause after the deal, while everybody reads their hand.
     *
     * The moment it started and how long it runs, never a countdown - a phone
     * with a clock four minutes out would otherwise show a pause already over,
     * or one that never ends. The screen counts locally from when it first sees
     * `startedAt`, the same way the challenge window works in Cheat.
     */
    settle:
      playing && state.settleUntil
        ? { startedAt: state.settleFrom, ms: state.settleUntil - state.settleFrom, totalMs: SETTLE_MS }
        : null,
